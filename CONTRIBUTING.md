# Contributing to RTSim

Thanks so much for your willingness to help out with RTSim's
...
Before you begin hacking on RTSim, you'll need to do a few things:

1. **Fork the Repository:**  On [RTSim's main page][homepage], in
   the top-right corner, you'll see a button to [fork the repository][fork].
   Do so, and then clone your fork and set it up with
   ```bash
   git clone git@github.com:${USER}/rtsim
   cd rtsim
   git remote add upstream git@github.com:sandialabs/rtsim
   git fetch upstream
   ```
2. **Set Up your Environment:**  Install the requirements with
   ```bash
   # Insert instructions for setting up a development environment.
   ```
3. Insert other steps as appropriate (e.g., run the examples, build the
   documentation, etc.)
   ```bash
   # Insert commands to run here.
   ```

[homepage]: https://github.com/sandialabs/rtsim
...
The [rtsim > Labels][labels] page shows you all the labels we use,
...
[labels]: https://github.com/sandialabs/rtsim/labels
...
Depending on the needs of the RTSim user base, a project maintainer
